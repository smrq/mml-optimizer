...
function noteNeighbors(token, state, options) {
	if (convert.roundVolume(token.volume, options) !==
		convert.roundVolume(state.volume, options))
		return [extend({}, state, { volume: token.volume })];
...
function optimizeTokens(mmlTokens, options) {
	var prunedTokens = mmlTokens.filter(function (token) {
		return ['note', 'rest', 'tempo', 'tie', 'nextVoice']
	...
	for (var i = 1; i < path.length; ++i) {
		...
		else if (nodeA.volume !== nodeB.volume)
			optimizedTokens.push({
				type: 'volume',
				volume: nodeB.volume,
				time: prunedTokens[nodeB.cursor].time
